...
use CDNServer\Core\EventListener\Base\ViewListenerInterface;
...
{
	private $watchedNamespace = 'CDNServer\\Core\\';
	private $watchedPrefix = 'CDNServerCore:';
	private $templating;
	private $engine;
	...
	public function onKernelView(GetResponseForControllerResultEvent $event)
	{
		if (is_array($event->getControllerResult()))
		{
			...
			$event->setResponse($this->templating->renderResponse(
					sprintf('CDNServerCore:%s:%s.%s.%s', $directory, $view, $request->getRequestFormat(), $this->engine),
